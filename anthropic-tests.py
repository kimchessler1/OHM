...
# See https://docs.anthropic.com/en/docs/quickstart for documentation and getting set up with a test API key
...
MODELS = [
    ...
    # "claude-3-5-sonnet-20240620", # DISABLED: to avoid incurring unnecessary costs when testing this script
    # "claude-3-opus-20240229" # EXCLUDED: this is already way too expensive
]
...
def summarize_results(responses):
    ...
    for response in responses:
        if response["included_content_in_prompt"] == True:
            input_tokens_with_content.append(response["response"].usage.input_tokens)
            output_tokens_with_content.append(response["response"].usage.output_tokens)
        else:
            input_tokens_without_content.append(response["response"].usage.input_tokens)
            output_tokens_without_content.append(response["response"].usage.output_tokens)
    ...
    for model in MODELS:
        ...
        for input_test_case in inputs:
            for include_textbook_content in [ True, False ]:
                ...
                model_json_output.append(response_json(response.content[0].text, model, input_test_case["LEARNING_OUTCOME"], include_textbook_content))
